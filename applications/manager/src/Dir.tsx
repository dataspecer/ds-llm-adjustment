...
const Row = ({ iri, parentIri }: { iri: string, parentIri?: string }) => {
  ...
  return <li className="first:border-y last:border-none border-b">
    <div className="flex items-center space-x-4 hover:bg-accent">
      ...
      {resource.userMetadata?.tags?.map((tag: string) => <Badge variant="secondary" key={tag}>{tag}</Badge>)}
      ...
      {resource.types.includes(V1.PSM) && <Button asChild variant={"ghost"} onClick={stopPropagation()}>
        <a href={(import.meta.env.VITE_VALIDATOR_UI || "http://localhost:3001") + "/?mode=maintainer&data-psm-schema=" + encodeURIComponent(iri) + "&data-specification=" + encodeURIComponent(parentIri || "")}>
          {t("validate")}
        </a>
      </Button>}
